...
def campaign18_result():
    return run(campaign_id=18, pre_days=586)
...
def test_small_sample_campaign_returns_insufficient_sample_status():
    # 캠페인 3: 순수 처치가구 0명으로 이미 확인된 캠페인 — 예외 없이 status 반환해야 함
    res = run(campaign_id=3, pre_days=90)
    assert res["status"] == "insufficient_sample"
    assert res["n_treatment"] == 0


def test_different_campaign_does_not_leak_campaign18_period():
    res = run(campaign_id=13, pre_days=90)
